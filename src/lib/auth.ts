  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        // For all other credentials, find user and assign USER role
        try {
          ...
          return {
            ...
            email: user.email,
            name: user.name,
            role: "USER", // Force USER role for all non-admin credentials
          };
        } catch (dbErr: any) {
          console.error('Database error during USER login:', dbErr?.message || dbErr);
  ...
  callbacks: {
    ...
    async signIn({ user, account }) {
      ...
      if (account?.provider === "google") {
        // Determine role: only admin@colortouch.app gets ADMIN role, all others are USER
